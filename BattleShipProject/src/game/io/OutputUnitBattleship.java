...
import graphicaluserinterface.GamePanel;
import graphicaluserinterface.GamePanel2;
...
public class OutputUnitBattleship extends JFrame implements OutputUnit {

	private static final long serialVersionUID = 8292954314171245311L;

	private InputListenerBattleship input;
	...
	public void publish(GameState state) {
		this.state = state;
		input = new InputListenerBattleship();
		input.inputOccured(new Move(state.getPlayerInTurn(), state
				.getPlayerInTurn().getPieces().get(0), new BoardLocation("A1")));
		initialize();
	}
